...
// const gravityValue = new Animated.Value(1);
const jumpValue = -15;
...
const jumpAnimation = () => {
  velocityY.setValue(jumpValue); // 터치 시 속도를 설정하여 튀어오르도록 합니다.
  Animated.timing(positionY, {
    toValue: -1, // 물체가 화면 위로 올라갈 최종 위치 (0은 화면 위를 나타냄)
    duration: 400, // 점프 애니메이션 지속 시간
    useNativeDriver: false, // 네이티브 드라이버 사용
  }).start(() => {
    // 점프 애니메이션 완료 후 중력 애니메이션 재시작
    gravityAnimation();
  });
};
```
